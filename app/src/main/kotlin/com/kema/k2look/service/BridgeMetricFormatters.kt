 ...
 *
 * All functions are package-internal and stateless.
 * They are called from KarooActiveLookBridge metric observers.
 */

/** Standard stream: format [singleValue] with a trailing [unit]. */
internal fun formatStreamData(streamState: StreamState?, unit: String): String = when (streamState) {
    is StreamState.Streaming    -> streamState.dataPoint.singleValue
        ?.let { "${formatValue(it)} $unit" } ?: "-- $unit"
...
internal fun formatTimeData(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming -> {
        val ms = streamState.dataPoint.singleValue?.toLong()
        if (ms != null) {
            val s = (ms / 1000) % 60; val m = (ms / 60_000) % 60; val h = ms / 3_600_000
...
internal fun formatZoneData(streamState: StreamState?, max: Int): String = when (streamState) {
    is StreamState.Streaming    -> streamState.dataPoint.singleValue?.toInt()
        ?.takeIf { it in 1..max }?.let { "Z$it" } ?: "--"
    is StreamState.Searching    -> "..."
...
/** Percentage (0-100) → "72%" */
internal fun formatPercent(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming    -> streamState.dataPoint.singleValue
        ?.let { "%.0f%%".format(it) } ?: "--%"
    is StreamState.Searching    -> "...%"
    is StreamState.Idle, null   -> "--%"
...
internal fun formatGrade(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming    -> streamState.dataPoint.singleValue?.let {
...
internal fun formatInteger(streamState: StreamState?): String = when (streamState) {
...
internal fun formatLapTime(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming -> {
        val ms = streamState.dataPoint.singleValue?.toLong()
        if (ms != null) {
            val s = (ms / 1000) % 60; val m = (ms / 60_000) % 60; val h = ms / 3_600_000
...
internal fun formatClockTime(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming -> {
        val ms = streamState.dataPoint.singleValue?.toLong()
        if (ms != null) {
            val totalMin = ms / 60_000; val h = (totalMin / 60) % 24; val m = totalMin % 60
...
internal fun formatDuration(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming -> {
...
internal fun formatDistanceToTurn(streamState: StreamState?): String = when (streamState) {
    is StreamState.Streaming    -> streamState.dataPoint.singleValue?.let {
...
internal fun formatGear(
    streamState: StreamState?,
    gearField: String,
    maxField: String
): String = when (streamState) {
    is StreamState.Streaming -> {
...
/**
 * Drivetrain battery from SHIFTING_BATTERY stream.
 * BatteryStatus ordinal: 0=New, 1=Good, 2=OK, 3=Low, 4=Critical, 5=Invalid
 * Prefers rear derailleur status; falls back to overall status.
 */
internal fun formatShiftingBattery(streamState: StreamState?): String {
    ...
    return when (streamState) {
        is StreamState.Streaming -> {
            val v   = streamState.dataPoint.values
            val raw = (v[DataType.Field.SHIFTING_BATTERY_STATUS_REAR_DERAILLEUR]
...
internal fun formatHeading(streamState: StreamState?): String {
    ...
    return when (streamState) {
        is StreamState.Streaming    -> dirs.getOrElse(
            streamState.dataPoint.singleValue?.toInt() ?: 8) { "--" }
        is StreamState.Searching    -> "..."
        is StreamState.Idle, null   -> "--"
        is StreamState.NotAvailable -> "N/A"
    }
